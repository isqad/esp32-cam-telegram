...
#define TG_API_URL "https://api.telegram.org/bot689500456:AAGl4r4t2j-LwcsVIYduzJJCR1MTZBCgYUY/getUpdates"
...
static esp_err_t http_client_event_handler(esp_http_client_event_t* event)
{
  ...
  case HTTP_EVENT_ON_CONNECTED:
    ESP_LOGI(TAG, "Successfully connected to %s", TG_API_URL);
    break;
  ...
  case HTTP_EVENT_ON_FINISH:
    ...
    ESP_LOGI(TAG, "Finish http session with %s", TG_API_URL);
    break;
  case HTTP_EVENT_DISCONNECTED:
    ESP_LOGW(TAG, "Disconnected from %s", TG_API_URL);
    if (http_response_buffer != NULL)
    {
      free(http_response_buffer);
      http_response_buffer = NULL;
